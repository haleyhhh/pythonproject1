...
from results.CIFAR10.pro import evaluate_model
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc3 = nn.Linear(256, 47)  # 修改为47个类别
...
def main():
    ...
    # 不使用隐私预算分配
    model_no_dp = SimpleCNN().to(device)
    optimizer_no_dp = optim.SGD(model_no_dp.parameters(), lr=0.01, momentum=0.9)
    for epsilon in epsilon_values:
        loss_no_dp, accuracy_no_dp = train_and_evaluate(model_no_dp, train_loader, test_loader, optimizer_no_dp,
                                                        epsilon)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('privacy_budget_comparison_femnist.png')  # 保存图片
    plt.show()
