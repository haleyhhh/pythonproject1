...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 * 8 * 8, 128)  # 输入特征图的大小为 8x8
        self.fc2 = nn.Linear(128, 10)  # CIFAR-10 有 10 个类别

    def forward(self, x):
        x = self.pool(torch.relu(self.conv1(x)))  # 输出特征图大小为 16x16
        x = self.pool(torch.relu(self.conv2(x)))  # 输出特征图大小为 8x8
...
# 逐步减少噪声的训练和评估模型的函数
def train_and_evaluate_model_noafl(model_noafl, optimizer_noafl, criterion, train_loader, test_loader, epochs, noise_scale):
    for epoch in range(epochs):
        model_noafl.train()
        for data, target in train_loader:
...
def main():
    ...
    for i, eps in enumerate(epsilons):
        ...
        # 训练并评估 DP-FL 模型
        accuracies_dp_fl[i] = train_and_evaluate_model(model_dp, optimizer_dp, criterion, train_loader, test_loader, 10, 1 / eps)

        # 训练并评估 LDP-FL 模型
        accuracies_ldp_fl[i] = train_and_evaluate_model(model_ldp, optimizer_ldp, criterion, train_loader, test_loader, 10, 1 / (eps * 0.5))

        # 训练并评估 NoAFL/DPBA 模型，逐步减少噪声
        accuracies_noafl[i] = train_and_evaluate_model_noafl(model_noafl, optimizer_noafl, criterion, train_loader, test_loader, 10, 1 / eps)
    ...
    plt.ylabel('Accuracy')
    plt.title('Accuracy vs Privacy Budget on CIFAR-10')
    plt.legend()
    plt.show()
